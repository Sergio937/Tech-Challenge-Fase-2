...
try:
    ...
    sqs_client = session.client("sqs", endpoint_url=os.getenv("AWS_SQS_ENDPOINT"))
    dynamodb_client = session.client("dynamodb", endpoint_url=os.getenv("AWS_DYNAMODB_ENDPOINT"))
...
def ensure_resources():
    ...
    try:
        sqs_client.create_queue(QueueName="analytics-queue")
        log.info("Fila SQS 'analytics-queue' criada (ou já existia).")
    except Exception as e:
        log.error(f"Erro ao criar fila SQS: {e}")
    ...
    try:
        ...
        log.info("Tabela DynamoDB 'analytics-events' criada.")
    except dynamodb_client.exceptions.ResourceInUseException:
        log.info("Tabela DynamoDB já existe.")
    except Exception as e:
